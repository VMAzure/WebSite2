...
<template>
  <section class="row">
    <header class="head">
      ...
      <div class="actions">
        ...
        <div class="nav" v-if="hasItems">
          <button class="navbtn" type="button" aria-label="Indietro" @click="scrollByCard(-1)">‹</button>
          <button class="navbtn" type="button" aria-label="Avanti" @click="scrollByCard(1)">›</button>
        </div>
      </div>
    </header>
    ...
    <div class="scroller" ref="scrollerRef">
      ...
      >
        <div class="tileInner">
          <img v-if="it.iconUrl" class="icon" :src="it.iconUrl" :alt="it.label" loading="lazy" />
...
<style scoped>
.row { width:100%; padding: clamp(1.2rem, 3vw, 2rem) 0; }
...
.title{ margin:0; font-size: clamp(1.15rem, 2.4vw, 1.55rem); font-weight:800; letter-spacing:-0.01em; }

.actions{ display:flex; align-items:center; gap: clamp(0.6rem, 2vw, 1rem); }
...
.nav{ display:inline-flex; gap: 0.5rem; }
...
.tileInner{ display:flex; flex-direction:column; align-items:center; gap: clamp(0.55rem, 1.6vw, 0.8rem); }
.icon{ width: clamp(3.6rem, 7vw, 4.6rem); height: clamp(3.0rem, 6vw, 3.8rem); object-fit:contain; display:block; }
.label{ font-size: clamp(0.85rem, 1.6vw, 0.98rem); font-weight:600; opacity:0.9; text-align:center; line-height:1.2; }
</style>
