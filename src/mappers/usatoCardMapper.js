...
export function mapToCardUsatoDto(item) {
    ...
    const allestimento = toStringSafe(
        item?.allestimento ?? item?.model ?? item?.Allestimento,
    );

    const anno_immatricolazione = toNumber(
        item?.anno_immatricolazione ?? item?.anno ?? item?.Anno,
    );
    const km_certificati = toNumber(item?.km_certificati ?? item?.km ?? item?.Km);
    const prezzo_vendita = toNumber(
        item?.prezzo_vendita ?? item?.prezzo ?? item?.Prezzo,
    );

    const cover_url =
        toStringSafe(
            item?.cover_url ?? item?.coverUrl ?? item?.immagine_copertina,
        ) || null;
