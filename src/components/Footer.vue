<template>
	<footer class="footer" :style="{ fontFamily: settings.font_family }">
		...
		<div class="divider" :style="{ backgroundColor: settings.tertiary_color }"></div>
...
<div class="wrapper">
  ...
  <button v-if="showPrefs" class="prefs-link" @click="openCookieBanner">Preferenze cookie</button>
</div>
    </div>

    <!-- MAPPA -->
    <div class="col map-col footer-map" v-if="settings.contact_address && canShowMap">
      <div :id="mapId" class="map-frame"></div>
    </div>

<div class="col map-col footer-map" v-else-if="settings.contact_address">
  <div
  ...
  >
    Per visualizzare la mappa devi accettare i cookie. <span class="map-cta">Apri preferenze</span>
  </div>
</div>
...
<script setup="">
    ...
    const cookieLink = computed(() => {
        const cfg = iubendaCfg.value
        if (!cfg?.cookieId) return null
        return `https://www.iubenda.com/privacy-policy/${cfg.cookieId}/cookie-policy`
    })
    ...
    const refreshConsent = () => {
        ...
        try {
            // ✅ caso 1: hai una funzione globale (es. la mette CookieBanner)
            if (typeof window.getConsent === "function") {
                consent.value = window.getConsent(slug.value)
                return
            }

            // ✅ caso 2: se in futuro importerai getConsent (qui non crasha)
            if (typeof getConsent === "function") {
                consent.value = getConsent(slug.value)
                return
            }

            // ✅ fallback: niente consenso -> niente terze parti
            consent.value = null
        } catch (e) {
            console.error("[Footer] refreshConsent error:", e)
            consent.value = null
        }
    }
    ...
    const openCookieBanner = () => {
        ...
        try {
            // ✅ 1) API Iubenda (se disponibile)
            if (typeof window._iub !== "undefined" && typeof window._iub.cs?.api?.openPreferences === "function") {
                window._iub.cs.api.openPreferences()
                return
            }

            // ✅ 2) fallback: click sul link/bottone che Iubenda inietta
            const el =
            ...
            // ✅ 3) ultimo fallback: evento custom (se in futuro lo gestisci altrove)
    ...
    const mapId = computed(() => `dealer-map-${props.settings?.google_place_id || "default"}`)
    ...
    const initMap = async () => {
        // ✅ se Leaflet non è stato caricato, non crashare mai
        if (typeof L === "undefined") {
            console.warn("[Footer] Leaflet (L) non caricato, skip map")
            return
        }

        if (!props.settings?.contact_address) return
        if (!canShowMap.value) return
        if (mapInstance.value) return // già creata
